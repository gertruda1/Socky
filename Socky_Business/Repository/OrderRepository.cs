...
namespace Socky_Business.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        {
            var orderHeader = await _db.OrderHeader.FindAsync(id);
            if (orderHeader == null)
            {
        ...
        public async Task<OrderDTO> Create(OrderDTO objDTO)
        {
            try
            {
                ...
                return new OrderDTO()
                {
                    ...
                    OrderDetails = _mapper.Map<IEnumerable<OrderDetails>, IEnumerable<OrderDetailsDTO>>(obj.OrderDetails).ToList()
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<int> Delete(int id)
        {
            ...
            if (objHeader != null)
            {
                IEnumerable<OrderDetails> objDetail = _db.OrderDetails.Where(u => u.OrderHeaderId == id);
        ...
        public async Task<OrderHeaderDTO> UpdateHeader(OrderHeaderDTO objDTO)
        {
            if (objDTO != null)
            {
                var orderHeaderFromDb = _db.OrderHeader.FirstOrDefault(u => u.Id==objDTO.Id);
